$.fn.powa4 = function(nbr_y, nbr_x, player1, player2, color_p1, color_p2) {
  ...
  function createGrid() {
    $("header").append(
      "<div onclick=document.location.href='index.html' class='playerTurn'><p>" +
        currentPlayer +
        " à toi de jouer !</p></div>"
    );
    $("div").css("background-color", color_p1);

    $("body").append(
      "<section class='players'><p class='" +
        player1 +
        "'>" +
        player1 +
        "</p>VS<p class='" +
    ...
    var playAgain = $("<button class='playAgain'>Rejouer</button>");
